...
ON_WM_CTLCOLOR()
ON_NOTIFY(LVN_ITEMCHANGED, IDC_LIST_VISITED,
          &CSaveDeviceInfoDlg::OnLvnItemchangedListVisited)
END_MESSAGE_MAP()
...
void CSaveDeviceInfoDlg::SiteVisitedThread() {
  ...
  do {
    ...
    map<CString, struct enz_packet_info>::iterator it =
        (*m_pmSiteVisited)[m_csIPAddress].begin();
    while ((it != (*m_pmSiteVisited)[m_csIPAddress].end()) &&
      ...
      if (it->second.csSite == it->first) {
        ...
        if ((csText != it->first) && (IsInTheList(csText) == -1)) {
          ...
          if (nRow == 0) {
            m_ctrlListVisited.InsertItem(LVIF_TEXT | LVIF_STATE, nRow,
                                         to_wstring(nRow + 1).c_str(), 0, 0, 0,
                                         0);
            ...
            m_ctrlListVisited.SetItemText(nRow, 4,
                                          to_wstring(it->second.nPort).c_str());
            ...
            for (int i = 0; i < m_ctrlListVisited.GetItemCount(); i++) {
              ...
              if ((ulItemBefore < UlIP) && (UlIP < ulItemAfter)) {
                ...
                break;
                         ...
                         (i == m_ctrlListVisited.GetItemCount() - 1)) {
                nRow = i + 1;
                break;
...
afx_msg LRESULT CSaveDeviceInfoDlg::OnSiteVisited(WPARAM wParam,
                                                  LPARAM lParam) {
  return 0;
}
BOOL CSaveDeviceInfoDlg::OnInitDialog() {
  ...
  LPCTSTR lpcRecHeader[] = {_T("No."),      _T("Time Visited"),
                            _T("Protocol"), _T("IP Address"),
                            _T("Port"),     _T("Site")};
  DWORD value = 0;
  DWORD BufferSize = 4;

  ::SetWindowTheme(GetDlgItem(IDC_STATIC_DEV_INFO)->GetSafeHwnd(), NULL,
                   _T(""));
  ::SetWindowTheme(GetDlgItem(IDC_STATIC_VISITED)->GetSafeHwnd(), NULL, _T(""));

  m_ctrlListVisited.SetExtendedStyle(LVS_EX_GRIDLINES | LVS_EX_FULLROWSELECT |
                                     LVS_EX_DOUBLEBUFFER);
...
bool CSaveDeviceInfoDlg::CallbackPacketListener(unsigned char *buffer,
                                                int nSize, void *pObject) {
...
CString CSaveDeviceInfoDlg::GetHostName(CString ip) {
  ...
  getnameinfo(addrs->ai_addr, (socklen_t)addrs->ai_addrlen, hostname,
              NI_MAXHOST, NULL, 0, 0);
...
bool CSaveDeviceInfoDlg::SaveDeviceName() {
  ...
  if (dwError == ERROR_SUCCESS) {
    LONG setRes = RegSetValueEx(hKey, csMacAddress, 0, REG_SZ,
                                (LPBYTE)csDevName.GetBuffer(),
                                sizeof(TCHAR) * csDevName.GetLength());
...
void CSaveDeviceInfoDlg::OnLvnItemchangedListVisited(NMHDR *pNMHDR,
                                                     LRESULT *pResult) {
...
void CSaveDeviceInfoDlg::DisplaySpeed(unsigned char *buffer, int nSize,
                                      void *pObj) {
  ...
  if ((timeCurrent - pDevice->m_ullTimeStarted) >= POLLING_TIME) {
    pDevice->m_lfUploadSpeed =
        ((double)pDevice->m_ullUploadSize /
         (double)(timeCurrent - pDevice->m_ullTimeStarted)) *
        8;
    pDevice->m_ullUploadSize = 0;

    pDevice->m_lfDownloadSpeed =
        ((double)pDevice->m_ullDownloadSize /
         (double)(timeCurrent - pDevice->m_ullTimeStarted)) *
        8;
