...
BOOL CCheckOpenPortsApp::IsAdministrator() {
	...
	if (!AllocateAndInitializeSid(&NtAuthority, 2, SECURITY_BUILTIN_DOMAIN_RID,
		...
		DEBUG_LOG(_T("Network Monitoring Tool : CheckTokenMembership() Error: ") +
				  to_wstring(dwError));
		goto Cleanup;
	}
...
void CCheckOpenPortsApp::ElevateProcess() {
	...
	try {
		...
		TCHAR szMessage[256];
		_stprintf_s(szMessage, ARRAYSIZE(szMessage),
			L"Error code returned was 0x%08lx", dwErrorCode);
		wstring s(szMessage);
...
int CCheckOpenPortsApp::ProcessAppToFirewall(LPCTSTR szAppName) {
	...
	if (firewall.InitializeCOM()) {
		...
		hr = firewall.WindowsFirewallAppIsEnabled(fwProfile, szFileNamePath,
			&bIsAppEnable);
		if (FAILED(hr)) {
		...
		if (!bIsAppEnable) {
			...
			int bRet = ::MessageBox(GetMainWnd()->GetSafeHwnd(), csMsg,
				_T("Enzo Tech Network Monitoring Tool"),
				MB_YESNO | MB_ICONQUESTION);
			if (IDYES == bRet) {
				hr = firewall.WindowsFirewallAddApp(fwProfile, szFileNamePath,
					szAppName);
				if (FAILED(hr)) {
					//		DEBUG_LOG(_T("WindowsFirewallAddApp failed: 0x%08lx\n"),
					// hr);
