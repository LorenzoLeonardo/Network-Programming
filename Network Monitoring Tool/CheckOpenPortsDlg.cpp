...
ON_WM_PAINT()
ON_WM_QUERYDRAGICON()
ON_BN_CLICKED(IDC_BUTTON_PORT,
	&CCheckOpenPortsDlg::OnBnClickedButtonStartSearchingOpenPort)
ON_BN_CLICKED(IDC_BUTTON2,
	&CCheckOpenPortsDlg::OnBnClickedButtonStopSearchingOpenPorts)
ON_BN_CLICKED(IDC_BUTTON_CHECKPORT,
	&CCheckOpenPortsDlg::OnBnClickedButtonCheckIfPortOpen)
ON_WM_CLOSE()
ON_EN_CHANGE(IDC_EDIT_AREA, &CCheckOpenPortsDlg::OnEnChangeEditArea)
ON_BN_CLICKED(IDC_BUTTON_LISTEN_LAN,
	&CCheckOpenPortsDlg::OnBnClickedButtonStartListenLan)
ON_BN_CLICKED(IDC_BUTTON_STOP_LAN,
	&CCheckOpenPortsDlg::OnBnClickedButtonStopListenLan)
...
ON_WM_CREATE()
ON_WM_CTLCOLOR()
ON_BN_CLICKED(IDC_BUTTON_START_PACKET,
	&CCheckOpenPortsDlg::OnBnClickedButtonStartPacket)
ON_BN_CLICKED(IDC_BUTTON_STOP_PACKET,
	&CCheckOpenPortsDlg::OnBnClickedButtonStopPacket)
ON_BN_CLICKED(IDC_BUTTON_SHOW_PACKETS,
	&CCheckOpenPortsDlg::OnBnClickedButtonShowPackets)
ON_WM_MOUSEMOVE()
ON_WM_MOVE()
ON_BN_CLICKED(IDC_CHECK_DEBUG, &CCheckOpenPortsDlg::OnBnClickedCheckDebug)
ON_CBN_SELCHANGE(IDC_COMBO_LIST_ADAPTER,
	&CCheckOpenPortsDlg::OnCbnSelchangeComboListAdapter)
ON_WM_MOVING()
ON_WM_LBUTTONDBLCLK()
ON_WM_RBUTTONDBLCLK()
ON_WM_RBUTTONDOWN()
ON_WM_RBUTTONUP()
ON_WM_NCMOUSELEAVE()
ON_BN_CLICKED(IDC_CHECK_INTERNET_ONLY,
	&CCheckOpenPortsDlg::OnBnClickedCheckInternetOnly)
ON_WM_NCLBUTTONDOWN()
ON_WM_NCLBUTTONUP()
ON_WM_NCMOUSEHOVER()
ON_WM_NCMOUSEMOVE()
...
unsigned __stdcall CCheckOpenPortsDlg::OpenPortListenerThread(void* parg) {
	...
	pDlg->m_pfnPtrEnumOpenPorts(CW2A(pDlg->m_IPAddress), MAX_PORT,
		CallBackEnumPort);
...
bool CCheckOpenPortsDlg::InitDLL() {
	...
	if (m_hDLLhandle) {
		m_pfnPtrEnumOpenPorts =
			(LPEnumOpenPorts)GetProcAddress(m_hDLLhandle, "EnumOpenPorts");
		m_pfnPtrIsPortOpen =
			(LPIsPortOpen)GetProcAddress(m_hDLLhandle, "IsPortOpen");
		...
		m_pfnPtrGetDefaultGateway =
			(FNGetDefaultGateway)GetProcAddress(m_hDLLhandle, "GetDefaultGateway");
		m_pfnPtrStopSearchingOpenPorts = (FNStopSearchingOpenPorts)GetProcAddress(
			m_hDLLhandle, "StopSearchingOpenPorts");
		m_pfnPtrGetNetworkDeviceStatus = (FNGetNetworkDeviceStatus)GetProcAddress(
			m_hDLLhandle, "GetNetworkDeviceStatus");
		m_pfnPtrEnumNetworkAdapters = (FNEnumNetworkAdapters)GetProcAddress(
			m_hDLLhandle, "EnumNetworkAdapters");
		m_pfnPtrGetDefaultGatewayEx = (FNGetDefaultGatewayEx)GetProcAddress(
			m_hDLLhandle, "GetDefaultGatewayEx");
		m_fnptrCreatePacketListenerEx = (FNPTRCreatePacketListenerEx)GetProcAddress(
			m_hDLLhandle, "CreatePacketListenerEx");
		m_fnptrStartPacketListenerEx = (FNPTRStartPacketListenerEx)GetProcAddress(
			m_hDLLhandle, "StartPacketListenerEx");
		m_fnptrStopPacketListenerEx = (FNPTRStopPacketListenerEx)GetProcAddress(
			m_hDLLhandle, "StopPacketListenerEx");
		m_fnptrDeletePacketListenerEx = (FNPTRDeletePacketListenerEx)GetProcAddress(
			m_hDLLhandle, "DeletePacketListenerEx");
		m_fnptrCreateLocalAreaListenerEx =
			(FNPTRCreateLocalAreaListenerEx)GetProcAddress(
				m_hDLLhandle, "CreateLocalAreaListenerEx");
		m_fnptrStartLocalAreaListenerEx =
			(FNPTRStartLocalAreaListenerEx)GetProcAddress(
				m_hDLLhandle, "StartLocalAreaListenerEx");
		m_fnptrStopLocalAreaListenerEx =
			(FNPTRStopLocalAreaListenerEx)GetProcAddress(m_hDLLhandle,
				"StopLocalAreaListenerEx");
		m_fnptrDeleteLocalAreaListenerEx =
			(FNPTRDeleteLocalAreaListenerEx)GetProcAddress(
				m_hDLLhandle, "DeleteLocalAreaListenerEx");
		m_fnptrSetNICAdapterToUse = (FNPTRSetNICAdapterToUse)GetProcAddress(
			m_hDLLhandle, "SetNICAdapterToUse");
		return true;
	}
	CString csMsg;
...
bool CCheckOpenPortsDlg::InitGUIControls() {
	LPCTSTR lpcRecHeader[] = { _T("No."),
		...
		_T("Download Speed"),
		_T("Upload Speed"),
		_T("Max Download Speed"),
		_T("Max Upload Speed") };
	DWORD value = 0;
	DWORD BufferSize = 4;

	::SetWindowTheme(GetDlgItem(IDC_STATIC_ROUTER_INFO)->GetSafeHwnd(), NULL,
		_T("")); // To change text Color of Group Box
	::SetWindowTheme(GetDlgItem(IDC_STATIC_ADAPTER_INFO)->GetSafeHwnd(), NULL,
		_T(""));
	::SetWindowTheme(GetDlgItem(IDC_STATIC_OPEN_PORTS)->GetSafeHwnd(), NULL,
		_T(""));
	::SetWindowTheme(GetDlgItem(IDC_CHECK_DEBUG)->GetSafeHwnd(), NULL, _T(""));
	::SetWindowTheme(GetDlgItem(IDC_CHECK_INTERNET_ONLY)->GetSafeHwnd(), NULL,
		_T(""));
	...
	m_ctrlLANConnected.InsertColumn(COL_NUMBER, lpcRecHeader[COL_NUMBER],
		LVCFMT_FIXED_WIDTH, 30);
	m_ctrlLANConnected.InsertColumn(COL_IPADDRESS, lpcRecHeader[COL_IPADDRESS],
		LVCFMT_LEFT, 100);
	m_ctrlLANConnected.InsertColumn(COL_DEVICENAME, lpcRecHeader[COL_DEVICENAME],
		LVCFMT_LEFT, 110);
	m_ctrlLANConnected.InsertColumn(COL_MACADDRESS, lpcRecHeader[COL_MACADDRESS],
		LVCFMT_LEFT, 0);
	m_ctrlLANConnected.InsertColumn(
		COL_DOWNLOADSPEED, lpcRecHeader[COL_DOWNLOADSPEED], LVCFMT_LEFT, 100);
	m_ctrlLANConnected.InsertColumn(
		COL_UPLOADSPEED, lpcRecHeader[COL_UPLOADSPEED], LVCFMT_LEFT, 100);
	m_ctrlLANConnected.InsertColumn(COL_DOWNLOADMAXSPEED,
		lpcRecHeader[COL_DOWNLOADMAXSPEED],
		LVCFMT_LEFT, 120);
	m_ctrlLANConnected.InsertColumn(
		COL_UPLOADMAXSPEED, lpcRecHeader[COL_UPLOADMAXSPEED], LVCFMT_LEFT, 120);
	...
	RegGetValue(HKEY_LOCAL_MACHINE, REGISTRY_PATH, _T("DebugLog"),
		/*RRF_RT_ANY*/ RRF_RT_DWORD, NULL, (PVOID)&value, &BufferSize);
	if (value)
...
bool CCheckOpenPortsDlg::InitNIC() {
	...
	m_fnptrSetNICAdapterToUse(
		m_vAdapterInfo[m_nCurrentNICSelect].AdapterInfo.AdapterName,
		m_vAdapterInfo[m_ctrlComboAdapterList.GetCurSel()]
			.AdapterInfo.IpAddressList.Context);
	if (m_pfnPtrGetDefaultGatewayEx(
			m_vAdapterInfo[m_nCurrentNICSelect].AdapterInfo.AdapterName,
			szDefaultGateWay, sizeof(szDefaultGateWay))) {
		m_ipFilter = m_vAdapterInfo[m_nCurrentNICSelect]
	...
	m_ctrlStaticNICListen.SetWindowText(
		CA2W(m_vAdapterInfo[m_nCurrentNICSelect].AdapterInfo.Description));
	m_ctrlLANConnected.SetAdapterIP(
		CA2W(m_vAdapterInfo[m_nCurrentNICSelect]
				 .AdapterInfo.IpAddressList.IpAddress.String)
			.m_szBuffer);
...
BOOL CCheckOpenPortsDlg::OnInitDialog() {
	...
	m_hNICPacketListener =
		m_fnptrCreatePacketListenerEx(CallbackNICPacketListener, NULL);
	m_hThreadRouter =
		(HANDLE)_beginthreadex(NULL, 0, RouterThread, this, 0, NULL);
	m_hThreadNICListener =
		(HANDLE)_beginthreadex(NULL, 0, NICListenerThread, this, 0, NULL);
...
HBRUSH CCheckOpenPortsDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) {
	...
	case CTLCOLOR_STATIC: {
		...
		if (id == IDC_EDIT_AREA || id == IDC_EDIT_PACKET_REPORT ||
			id == IDC_EDIT_SPEED_DOWN || id == IDC_EDIT_SPEED_UP ||
			id == IDC_EDIT_ADAPTER_INFO) {
...
void CCheckOpenPortsDlg::OnBnClickedButtonStartSearchingOpenPort() {
	...
	m_hThreadOpenPortListener =
		(HANDLE)_beginthreadex(NULL, 0, OpenPortListenerThread, this, 0, NULL);
}
...
void CCheckOpenPortsDlg::OnClose() {
	...
	if (IsLANStopped() && IsSearchingOpenPortStopped()) {
		...
		::MessageBox(this->GetSafeHwnd(),
			_T("Application is still busy. Make sure all listening ")
			_T("activity has stopped."),
			_T("Network Monitoring Tool"), MB_ICONEXCLAMATION);
	}
}
...
void CCheckOpenPortsDlg::OnBnClickedButtonStartListenLan() {
	...
	string ip = m_vAdapterInfo[m_nCurrentNICSelect]
					.AdapterInfo.IpAddressList.IpAddress.String;
	string mask = m_vAdapterInfo[m_nCurrentNICSelect]
					  .AdapterInfo.IpAddressList.IpMask.String;

	m_fnptrStartLocalAreaListenerEx(
		m_vAdapterInfo[m_nCurrentNICSelect].hLANListener, ip.c_str(),
		mask.c_str(), CallbackLANListenerEx, POLLING_TIME);
}
...
void CCheckOpenPortsDlg::OnNMClickListLan(NMHDR* pNMHDR, LRESULT* pResult) {
	...
	*pResult = 0;
	...
	if (pNMItemActivate->iItem > -1) {
		// m_ipFilter = ;
		m_ctrlIPAddress.SetWindowText(
			m_ctrlLANConnected.GetItemText(m_nCurrentRowSelected, 1));
...
void CCheckOpenPortsDlg::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
	...
	m_ctrlLANConnected.SetItemState(m_nCurrentRowSelected, LVIS_SELECTED,
		LVIS_SELECTED);
	m_ctrlLANConnected.SetFocus();
	CDialogEx::OnKeyDown(nChar, nRepCnt, nFlags);
}

void CCheckOpenPortsDlg::OnHdnItemKeyDownListLan(NMHDR* pNMHDR,
	LRESULT* pResult) {
	// TODO: Add your control notification handler code here
...
void CCheckOpenPortsDlg::OnNMDblclkListLan(NMHDR* pNMHDR, LRESULT* pResult) {
	...
	*pResult = 0;
	...
	if (pNMItemActivate->iItem >= 0) {
		...
		CString csIP =
			m_ctrlLANConnected.GetItemText(pNMItemActivate->iItem, COL_IPADDRESS);
		ULONG ulIP;
		...
		if (nPtr == IDOK) {
			if (m_mConnected.find(ulIP) != m_mConnected.end()) {
				...
				m_ctrlLANConnected.SetItemText(pNMItemActivate->iItem, COL_DEVICENAME,
					saveDlg.GetDeviceName());
			}
		}
...
unsigned __stdcall CCheckOpenPortsDlg::RouterThread(void* parg) {
	...
	if (pDlg->m_vAdapterInfo.empty())
		return 0;
	if (pDlg->m_pfnPtrGetDefaultGatewayEx(
			pDlg->m_vAdapterInfo[pDlg->m_ctrlComboAdapterList.GetCurSel()]
...
#ifdef UNICODE
			sTemp = (char*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
				sizeof(char) * (value.value.sNumber + 1));
			if (sTemp) {
				memset(sTemp, 0, sizeof(char) * (value.value.sNumber + 1));
				memcpy_s(sTemp, sizeof(char) * (value.value.sNumber),
					value.value.string.ptr, value.value.sNumber);
				csBrand = CA2W(sTemp);
		...
		while (!pDlg->HasClickClose()) {
			...
			csFormat.Format(_T("%s %s %s\r\n\r\nRouter's Up Time\r\n%u days, %u ")
							_T("hours, %u min, %u secs"),
				csBrand.GetBuffer(), csModel.GetBuffer(),
				csDesc.GetBuffer(), ulDays, ulHour, ulMin, ulSec);
...
int CCheckOpenPortsDlg::IsInTheList(CString csIPAddress) {
	...
	for (int i = 0; i < m_ctrlLANConnected.GetItemCount(); ++i) {
		CString szText =
			g_pCCheckOpenPortsDlg->m_ctrlLANConnected.GetItemText(i, 1);
...
void CCheckOpenPortsDlg::UpdateDeviceConnected() {
	CString csConn;
	csConn.Format(_T("No. Device Connected: %d"),
		m_ctrlLANConnected.GetItemCount());
	m_ctrlStaticNumDevice.SetWindowTextW(csConn);
}

void CCheckOpenPortsDlg::ProcessLANListener(const char* ipAddress,
	const char* hostName,
	...
	if (bIsopen) {
		...
		if ((m_mConnected.find(ipaddr) == m_mConnected.end()) &&
			strlen(ipAddress)) {
			CString csTemp, format;
			CDeviceConnected* tDeviceDetails = new CDeviceConnected();

			tDeviceDetails->m_szIPAddress.Format(_T("%s"),
				CA2W(ipAddress).m_szBuffer);
			tDeviceDetails->m_szMACAddress.Format(_T("%s"),
				CA2W(macAddress).m_szBuffer);
			...
			tDeviceDetails->m_ullDownloadStartTime =
				tDeviceDetails->m_ullUploadStartTime = GetTickCount64();
			tDeviceDetails->m_hPacketListenerDownload = m_fnptrCreatePacketListenerEx(
				CallbackPacketListenerDownloadEx, (void*)tDeviceDetails);
			tDeviceDetails->m_hPacketListenerUpload = m_fnptrCreatePacketListenerEx(
				CallbackPacketListenerUploadEx, (void*)tDeviceDetails);
			...
			if (tDeviceDetails->m_szIPAddress == tDeviceDetails->m_szHostName) {
				...
				DWORD dwRet = RegGetValue(
					HKEY_LOCAL_MACHINE, REGISTRY_PATH, tDeviceDetails->m_szMACAddress,
					/*RRF_RT_ANY*/ RRF_RT_REG_SZ, NULL, (PVOID)&value, &BufferSize);
			...
			if (nRow == 0) {
				...
				m_ctrlLANConnected.SetItemText(nRow, col + 1,
					tDeviceDetails->m_szIPAddress);
				m_ctrlLANConnected.SetItemText(nRow, col + 2,
					tDeviceDetails->m_szHostName);
				m_ctrlLANConnected.SetItemText(nRow, col + 3,
					tDeviceDetails->m_szMACAddress);
				if (tDeviceDetails->m_lfDownloadSpeed <= 1000)
					format.Format(_T("%.2f Kbps"), tDeviceDetails->m_lfDownloadSpeed);
				else
					format.Format(_T("%.2f Mbps"),
						tDeviceDetails->m_lfDownloadSpeed / 1000);
				...
				if (tDeviceDetails->m_lfUploadSpeed <= 1000)
					...
					format.Format(_T("%.2f Mbps"),
						tDeviceDetails->m_lfUploadSpeed / 1000);
				...
				if (tDeviceDetails->m_lfMaxDownloadSpeed <= 1000)
					...
					format.Format(_T("%.2f Mbps"),
						tDeviceDetails->m_lfMaxDownloadSpeed / 1000);
				...
				if (tDeviceDetails->m_lfMaxUploadSpeed <= 1000)
					...
					format.Format(_T("%.2f Mbps"),
						tDeviceDetails->m_lfMaxUploadSpeed / 1000);
				m_ctrlLANConnected.SetItemText(nRow, col + 7, format);
			}
			else {
				for (int i = 0; i < m_ctrlLANConnected.GetItemCount(); i++) {
					ULONG ulItemBefore = 0, UlIP = 0, ulItemAfter = 0;
					InetPtonW(AF_INET, m_ctrlLANConnected.GetItemText(i, 1),
						&ulItemBefore);
					InetPtonW(AF_INET, m_ctrlLANConnected.GetItemText(i + 1, 1),
						&ulItemAfter);
					InetPtonW(AF_INET, tDeviceDetails->m_szIPAddress, &UlIP);
					if ((htonl(ulItemBefore) < htonl(UlIP)) &&
						(htonl(UlIP) < htonl(ulItemAfter))) {
						nRow = i + 1;
						break;
					}
					else if (htonl(ulItemBefore) < htonl(UlIP) &&
							 i == m_ctrlLANConnected.GetItemCount() - 1) {
						nRow = i + 1;
						break;
					}
					else if ((htonl(ulItemBefore) > htonl(UlIP)) &&
				...
				for (int j = nRow;
					 j < g_pCCheckOpenPortsDlg->m_ctrlLANConnected.GetItemCount();
					 j++) {
		...
		if (strcmp(ipAddress, "end") == 0) {
			...
			if (!m_mMonitorDeviceBefore.empty()) {
				...
				while (it != m_mMonitorDeviceCurrent.end()) {
					if (m_mMonitorDeviceBefore.find(it->first) !=
						m_mMonitorDeviceBefore.end())
				...
				while (it != m_mMonitorDeviceBefore.end()) {
					...
					if (nRow != -1) {
						...
						m_fnptrDeletePacketListenerEx(
							m_mConnected[it->first]->m_hPacketListenerDownload);
						m_fnptrDeletePacketListenerEx(
							m_mConnected[it->first]->m_hPacketListenerUpload);
			...
			CString csIP(CA2W(m_vAdapterInfo[m_nCurrentNICSelect]
								  .AdapterInfo.IpAddressList.IpAddress.String));
			m_ctrlLANConnected.SetAdapterIP(csIP);
			m_fnptrSetNICAdapterToUse(
				m_vAdapterInfo[m_nCurrentNICSelect].AdapterInfo.AdapterName,
				m_vAdapterInfo[m_nCurrentNICSelect]
					.AdapterInfo.IpAddressList.Context);
			m_ctrlStaticNICListen.SetWindowText(
				CA2W(m_vAdapterInfo[m_nCurrentNICSelect].AdapterInfo.Description));
...
void CCheckOpenPortsDlg::CallbackLANListenerEx(const char* ipAddress,
	const char* hostName,
	const char* macAddress,
	bool bIsopen) {
	mtx_lanlistener.lock();
	g_pCCheckOpenPortsDlg->ProcessLANListener(ipAddress, hostName, macAddress,
		bIsopen);
	mtx_lanlistener.unlock();
}

void CCheckOpenPortsDlg::CallBackEnumPort(char* ipAddress, int nPort,
	bool bIsopen, int nLastError) {
	if (ipAddress != NULL) {
		if (strcmp(ipAddress, "DONE") == 0) {
			...
			if (bIsopen) {
				...
				long nLength =
					g_pCCheckOpenPortsDlg->m_ctrlResult.GetWindowTextLength();
				g_pCCheckOpenPortsDlg->m_ctrlResult.SetSel(0, 0);
...
void CCheckOpenPortsDlg::CallBackEnumAdapters(void* args) {
	...
	for (int i = 0; i < g_pCCheckOpenPortsDlg->m_vAdapterInfo.size(); i++) {
		if (strcmp(g_pCCheckOpenPortsDlg->m_vAdapterInfo[i].AdapterInfo.AdapterName,
				pAdapterInfo->AdapterName) == 0) {
			bFound = true;
			nIndexFound = i;
			break;
		}
	...
	if (!bFound) {
		if (pAdapterInfo->IpAddressList.Context &&
			pAdapterInfo->GatewayList.Context) {
			CNetworkInterfaceInfo nicInfo;
			nicInfo.AdapterInfo = *pAdapterInfo;
			nicInfo.hLANListener =
				g_pCCheckOpenPortsDlg->m_fnptrCreateLocalAreaListenerEx();
			g_pCCheckOpenPortsDlg->m_vAdapterInfo.push_back(nicInfo);
			g_pCCheckOpenPortsDlg->m_ctrlComboAdapterList.AddString(
				CA2W(pAdapterInfo->Description));
		...
		if (!pAdapterInfo->IpAddressList.Context) {
			g_pCCheckOpenPortsDlg->m_fnptrStopLocalAreaListenerEx(
				g_pCCheckOpenPortsDlg->m_vAdapterInfo[nIndexFound].hLANListener);
			g_pCCheckOpenPortsDlg->m_fnptrDeleteLocalAreaListenerEx(
				g_pCCheckOpenPortsDlg->m_vAdapterInfo[nIndexFound].hLANListener);
			g_pCCheckOpenPortsDlg->m_vAdapterInfo.erase(
				g_pCCheckOpenPortsDlg->m_vAdapterInfo.begin() + nIndexFound);
...
void CCheckOpenPortsDlg::UpdateAdapterChanges() {
	...
	memset(szDefaultGateWay, 0, sizeof(szDefaultGateWay));
	if (!m_vAdapterInfo.empty()) {
		if (m_pfnPtrGetDefaultGatewayEx(m_vAdapterInfo[i].AdapterInfo.AdapterName,
				szDefaultGateWay,
				sizeof(szDefaultGateWay))) {
			m_ipFilter =
				CA2W(m_vAdapterInfo[i].AdapterInfo.IpAddressList.IpAddress.String);
			inet_pton(AF_INET,
			...
			// memcpy_s(p, sizeof(p), m_vAdapterInfo[i].AdapterInfo.Address,
			// sizeof(p));
			csWindowText.AppendFormat("Adapter Addr: \t%X:%X:%X:%X:%X:%X\r\n",
				m_vAdapterInfo[i].AdapterInfo.Address[0],
				m_vAdapterInfo[i].AdapterInfo.Address[1],
				m_vAdapterInfo[i].AdapterInfo.Address[2],
				m_vAdapterInfo[i].AdapterInfo.Address[3],
				m_vAdapterInfo[i].AdapterInfo.Address[4],
				m_vAdapterInfo[i].AdapterInfo.Address[5]);
			csWindowText.AppendFormat(
				"IP Addr: \t\t%s\r\n",
				m_vAdapterInfo[i].AdapterInfo.IpAddressList.IpAddress.String);
			csWindowText.AppendFormat(
				"IP Mask: \t\t%s\r\n",
		...
		AfxMessageBox(_T("All Network Interfaces are disconnected. Please check ")
					  _T("your ethernet ports or WIFI adapters."));
	}
}
bool CCheckOpenPortsDlg::CallbackNICPacketListener(unsigned char* buffer,
	int nSize, void* obj) {
	return g_pCCheckOpenPortsDlg->ProcessNICPacketListener(buffer, nSize, obj);
}
bool CCheckOpenPortsDlg::ProcessNICPacketListener(unsigned char* buffer,
	int nSize, void* obj) {
	...
	if (IsInternetOnly()) {
		...
		if (((ntohl(iphdr->unDestaddress) > dwBeginIP) &&
				(ntohl(iphdr->unDestaddress) < dwEndIP)) &&
			((ntohl(iphdr->unSrcaddress) > dwBeginIP) &&
				(ntohl(iphdr->unSrcaddress) < dwEndIP))) {
			return false;
		}
	}
	...
	if (m_pmodeless == NULL) {
		...
		inet_ntop(AF_INET, (const void*)&iphdr->unDestaddress, sztemp,
			sizeof(sztemp));
		destIP = sztemp;
		inet_ntop(AF_INET, (const void*)&iphdr->unSrcaddress, sztemp,
			sizeof(sztemp));
		sourceIP = sztemp;
		...
		if (GetIPFilterULONG() == iphdr->unDestaddress) {
			csReport = csText + sourceIP + _T(":") + csSrcPort + _T("->") + destIP +
					   _T(":") + csDestPort + _T("\t\tSize: ") +
					   to_wstring(nSize).c_str() + _T(" bytes\r\n");
			if (m_pmodeless)
...
void CCheckOpenPortsDlg::OnMove(int x, int y) {
	...
	if (m_pmodeless) {
		...
		// m_pmodeless->MoveWindow(x + rectParent.right, y-32, m_rectModeless.right,
		// m_rectModeless.bottom+3);
		RECT thisRect;
		GetDlgItem(IDC_LIST_LAN)->GetWindowRect(&thisRect);
		m_pmodeless->MoveWindow(thisRect.left, thisRect.top,
...
void CCheckOpenPortsDlg::OnBnClickedCheckDebug() {
	...
	dwError =
		RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGISTRY_PATH, 0, KEY_ALL_ACCESS, &hKey);
	if (dwError == ERROR_FILE_NOT_FOUND)
		dwError = RegCreateKeyEx(HKEY_LOCAL_MACHINE, REGISTRY_PATH, NULL, NULL,
	...
	if (dwError == ERROR_SUCCESS) {
		...
		if (ChkBox == BST_UNCHECKED) {
			...
			value = 0;
			m_ctrlBtnDebug.SetCheck(BST_UNCHECKED);
			LONG setRes = RegSetValueEx(hKey, _T("DebugLog"), 0, RRF_RT_DWORD,
				(LPBYTE)&value, BufferSize);
			AfxMessageBox(_T("Debugging log of the tool is disabled."),
				MB_ICONINFORMATION);
			...
			value = 1;
			m_ctrlBtnDebug.SetCheck(BST_CHECKED);
			LONG setRes = RegSetValueEx(hKey, _T("DebugLog"), 0, RRF_RT_REG_DWORD,
				(LPBYTE)&value, BufferSize);
			AfxMessageBox(_T("Debugging log of the tool is enabled."),
				MB_ICONINFORMATION);
			if (setRes == ERROR_SUCCESS)
				goto REGCLOSE;
			else
...
void CCheckOpenPortsDlg::OnCbnSelchangeComboListAdapter() {
	...
	if (!m_vAdapterInfo.empty()) {
		if (m_pfnPtrGetDefaultGatewayEx(m_vAdapterInfo[i].AdapterInfo.AdapterName,
			...
			csWindowText.AppendFormat("Adapter Desc: \t%s\r\n",
				m_vAdapterInfo[i].AdapterInfo.Description);
			memcpy(p, m_vAdapterInfo[i].AdapterInfo.Address,
				sizeof(m_vAdapterInfo[i].AdapterInfo.Address));
			csWindowText.AppendFormat("Adapter Addr: \t%X:%X:%X:%X:%X:%X\r\n", p[0],
				p[1], p[2], p[3], p[4], p[5]);
			csWindowText.AppendFormat(
				"IP Addr: \t\t%s\r\n",
...
inline void CCheckOpenPortsDlg::DisplayDownloadSpeed(CString szIPAddress,
	int nColumn,
	double ldData) {
	CString csFormat;
	...
	if (findResult != -1) {
		...
		if (m_ctrlLANConnected.GetItemText(findResult, nColumn).Compare(csFormat) !=
			0) {
...
inline void CCheckOpenPortsDlg::DisplayUploadSpeed(CString szIPAddress,
	int nColumn, double ldData) {
	CString csFormat;
	LVFINDINFO lvFindInfo;
	LVITEM lvItem;
	int findResult = -1;
...
bool CCheckOpenPortsDlg::CallbackPacketListenerDownloadEx(unsigned char* buffer,
	int nSize,
	void* pObject) {
	return g_pCCheckOpenPortsDlg->ProcessPacketListenerDownloadEx(buffer, nSize,
		pObject);
}
bool CCheckOpenPortsDlg::ProcessPacketListenerDownloadEx(unsigned char* buffer,
	int nSize,
	void* pObject) {
	...
	if ((timeCurrent - pDevice->m_ullDownloadStartTime) >= POLLING_TIME) {
		pDevice->m_lfDownloadSpeed =
			((double)pDevice->m_ulDataSizeDownload /
				(double)(timeCurrent - pDevice->m_ullDownloadStartTime)) *
			8;
		if (pDevice->m_lfMaxDownloadSpeed < pDevice->m_lfDownloadSpeed) {
			pDevice->m_lfMaxDownloadSpeed = pDevice->m_lfDownloadSpeed;
			DisplayDownloadSpeed(pDevice->m_szIPAddress, COL_DOWNLOADMAXSPEED,
				pDevice->m_lfMaxDownloadSpeed);
		}
		pDevice->m_ulDataSizeDownload = 0;
		DisplayDownloadSpeed(pDevice->m_szIPAddress, COL_DOWNLOADSPEED,
			pDevice->m_lfDownloadSpeed);
		pDevice->m_ullDownloadStartTime = GetTickCount64();
	}
	return true;
}
bool CCheckOpenPortsDlg::CallbackPacketListenerUploadEx(unsigned char* buffer,
	int nSize,
	void* pObject) {
	return g_pCCheckOpenPortsDlg->ProcessPacketListenerUploadEx(buffer, nSize,
		pObject);
}

bool CCheckOpenPortsDlg::ProcessPacketListenerUploadEx(unsigned char* buffer,
	int nSize,
	void* pObject) {
	CDeviceConnected* pDevice = (CDeviceConnected*)pObject;
	...
	if (pDevice->m_szIPAddress == sourceIP) {
		...
		if (!(((ntohl(iphdr->unDestaddress) > dwBeginIP) &&
			...
			if (iphdr->ucIPProtocol == TCP_PROTOCOL) {
				...
				m_mSiteVisited[sourceIP].insert(m_mSiteVisited[sourceIP].begin(),
					mpair);
			}
	...
	if ((timeCurrent - pDevice->m_ullUploadStartTime) >= POLLING_TIME) {
		pDevice->m_lfUploadSpeed =
			((double)pDevice->m_ulDataSizeUpload /
				(double)(timeCurrent - pDevice->m_ullUploadStartTime)) *
			8;
		if (pDevice->m_lfMaxUploadSpeed < pDevice->m_lfUploadSpeed) {
			pDevice->m_lfMaxUploadSpeed = pDevice->m_lfUploadSpeed;
			DisplayUploadSpeed(pDevice->m_szIPAddress, COL_UPLOADMAXSPEED,
				pDevice->m_lfMaxUploadSpeed);
		}
		pDevice->m_ulDataSizeUpload = 0;
		DisplayUploadSpeed(pDevice->m_szIPAddress, COL_UPLOADSPEED,
			pDevice->m_lfUploadSpeed);
...
void CCheckOpenPortsDlg::OnBnClickedCheckInternetOnly() {
	...
	if (ChkBox == BST_UNCHECKED) {
		...
		m_bIsInternetOnly = false;
		AfxMessageBox(_T("The tool will be able to listen both local area and ")
					  _T("internet packets."),
			MB_ICONINFORMATION);
	}
	else {
		...
		AfxMessageBox(_T("The tool will be able to listen internet packets only."),
			MB_ICONINFORMATION);
	}
}
...
void CCheckOpenPortsDlg::EnableCloseButton(bool bEnable) {
	UINT nMenuf =
		bEnable ? (MF_BYCOMMAND) : (MF_BYCOMMAND | MF_GRAYED | MF_DISABLED);
