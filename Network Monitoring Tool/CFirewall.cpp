...
CFirewall::CFirewall() { m_comInit = E_FAIL; }
CFirewall::~CFirewall() {}
...
HRESULT
CFirewall::WindowsFirewallAppIsEnabled(IN INetFwProfile* fwProfile,
	...
	if (SUCCEEDED(hr)) {
		...
		if (fwEnabled != VARIANT_FALSE) {
			...
			*fwAppEnabled = TRUE;
			...
			DEBUG_LOG(_T("Authorized application %lS is enabled in the firewall.\n"),
				fwProcessImageFileName);
		}
		else {
			DEBUG_LOG(_T("Authorized application %lS is disabled in the firewall.\n"),
				fwProcessImageFileName);
		}
	}
	else {
...
HRESULT
CFirewall::WindowsFirewallAddApp(IN INetFwProfile* fwProfile,
	IN const wchar_t* fwProcessImageFileName,
	...
	hr = WindowsFirewallAppIsEnabled(fwProfile, fwProcessImageFileName,
		&fwAppEnabled);
	if (FAILED(hr)) {
	...
	if (!fwAppEnabled) {
		...
		hr = CoCreateInstance(
			__uuidof(NetFwAuthorizedApplication), NULL, CLSCTX_INPROC_SERVER,
...
HRESULT CFirewall::WindowsFirewallPortIsEnabled(
	IN INetFwProfile* fwProfile, IN LONG portNumber,
...
HRESULT CFirewall::WindowsFirewallPortAdd(IN INetFwProfile* fwProfile,
	IN LONG portNumber,
