...
void CPacketInfoDlg::UpdatePacketInfo(CString csPacketInfo, int nProtocol) {
  try {
    ...
    if (m_ctrlEditPacketReportArea.m_hWnd) {
      if (m_ctrlEditPacketReportArea.m_hWnd &&
          ((CButton *)GetDlgItem(IDC_CHECK_IGMP))->GetCheck() ==
              BST_UNCHECKED &&
          nProtocol == IGMP_PROTOCOL)
        return;
      else if (m_ctrlEditPacketReportArea.m_hWnd &&
               ((CButton *)GetDlgItem(IDC_CHECK_TCP))->GetCheck() ==
                   BST_UNCHECKED &&
               nProtocol == TCP_PROTOCOL)
        return;
      else if (m_ctrlEditPacketReportArea.m_hWnd &&
               ((CButton *)GetDlgItem(IDC_CHECK_UDP))->GetCheck() ==
                   BST_UNCHECKED &&
               nProtocol == UDP_PROTOCOL)
        return;
      else if (m_ctrlEditPacketReportArea.m_hWnd &&
               ((CButton *)GetDlgItem(IDC_CHECK_ICMP))->GetCheck() ==
                   BST_UNCHECKED &&
...
BOOL CPacketInfoDlg::OnInitDialog() {
  ...
  ((CCheckOpenPortsDlg *)m_pParent)
      ->GetDlgItem(IDC_LIST_LAN)
      ->GetWindowRect(&thisRect);
  // GetClientRect(&thisRect);
  MoveWindow(thisRect.left, thisRect.top, thisRect.right - thisRect.left,
  ...
  // m_hPacketInfoThread = (HANDLE)_beginthreadex(NULL, 0, PacketInfoThread,
  // this, 0, NULL);
...
HBRUSH CPacketInfoDlg::OnCtlColor(CDC *pDC, CWnd *pWnd, UINT nCtlColor) {
  ...
  case CTLCOLOR_BTN: {
    int id = pWnd->GetDlgCtrlID();
    if (id == IDC_CHECK_IGMP || id == IDC_CHECK_ICMP || id == IDC_CHECK_TCP ||
        id == IDC_CHECK_UDP) {
      // pDC->SetTextColor(RGB(255, 255, 255));
