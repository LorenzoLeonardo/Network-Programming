...
typedef bool (*FNStartLocalAreaListening)(const char *ipAddress,
                                          const char *subnetMask,
                                          FNCallbackLocalAreaListener fnpPtr,
                                          int nPollingTime);
typedef void (*FNStopLocalAreaListening)();
typedef bool (*FNStartSNMP)(const char *szAgentIPAddress,
                            const char *szCommunity, int nVersion,
                            DWORD &dwLastError);
...
typedef bool (*FNGetDefaultGatewayEx)(const char *szAdapterName,
                                      char *szDefaultGateway, int nSize);
...
typedef bool (*FNGetNetworkDeviceStatus)(const char *ipAddress, char *hostname,
                                         int nSizeHostName, char *macAddress,
                                         int nSizeMacAddress, DWORD *pError);
typedef bool (*FNEnumNetworkAdapters)(FNCallbackAdapterList);
typedef HANDLE (*FNPTRCreatePacketListenerEx)(FNCallbackPacketListenerEx,
                                              void *);
...
typedef bool (*FNPTRStartLocalAreaListenerEx)(
    HANDLE, const char *ipAddress, const char *subNetMask,
    FNCallbackLocalAreaListener fnpPtr, int nPollingTimeMS);
typedef void (*FNPTRStopLocalAreaListenerEx)(HANDLE);
typedef void (*FNPTRDeleteLocalAreaListenerEx)(HANDLE &);
typedef void (*FNPTRSetNICAdapterToUse)(const char *szAdapterName,
                                        ULONG ipAddress);

template <typename Map> inline bool key_compare(Map const &lhs, Map const &rhs);
...
// Dialog Data
  ...
  inline void DisplayUploadSpeed(CString szIPAddress, int nColumn,
                                 double ldData);
  inline void DisplayDownloadSpeed(CString szIPAddress, int nColumn,
                                   double ldData);
  ...
  bool HasClickClose() { return m_bHasClickClose; }

  void SetLANStop(bool b) { m_bLanStop = b; }
  void SetStopSearchingOpenPort() { m_bStopSearchingOpenPorts = true; }
  bool IsSearchingOpenPortStopped() { return m_bStopSearchingOpenPorts; }
  bool IsPacketStopped() { return m_bStopPacketListener; }
  bool IsLANStopped() { return m_bLanStop; }
  CString GetIPAddress() { return m_IPAddress; }
  void SetRouterUpTime(CString cs) {
    m_ctrlStaticRouterUpTime.SetWindowText(cs);
  }
  bool ShowPacketInfo() { return m_bShowPacketInfo; }
  DWORD GetIPFilterULONG() { return m_ulIPFilter; }

  bool IsInternetOnly() { return m_bIsInternetOnly; }
...
protected:
  ...
  static void CallbackLANListenerEx(const char *ipAddress, const char *hostName,
                                    const char *macAddress, bool bIsopen);
  static void CallBackEnumPort(char *ipAddress, int nPort, bool bIsopen,
                               int nLastError);
  static bool CallbackNICPacketListener(unsigned char *buffer, int nSize,
                                        void *obj);
  static void CallBackEnumAdapters(void *);
  static bool CallbackPacketListenerDownloadEx(unsigned char *buffer, int nSize,
                                               void *pObject);
  static bool CallbackPacketListenerUploadEx(unsigned char *buffer, int nSize,
                                             void *pObject);
  void ProcessLANListener(const char *ipAddress, const char *hostName,
                          const char *macAddress, bool bIsopen);
  bool ProcessPacketListenerDownloadEx(unsigned char *buffer, int nSize,
                                       void *pObject);
  bool ProcessNICPacketListener(unsigned char *buffer, int nSize, void *obj);
  bool ProcessPacketListenerUploadEx(unsigned char *buffer, int nSize,
                                     void *pObject);
